...
class MainWindow(QMainWindow):
    ...
    def on_work_order_row_double_clicked(self, index):
        ...
        # 1. 拍摄完成：摄影完成"上传素材"
        for log in logs:
            if "摄影" in log.get('role', '') and log.get('action_type', '') == "上传素材":
                finished_steps.add("拍摄完成")
                break
        # 2. 美工分发：美工分发运营或分发销售（模糊匹配）
        for log in logs:
            if "美工" in log.get('role', '') and "分发" in (log.get('action_type', '') + log.get('details', '')) and ("运营" in (log.get('action_type', '') + log.get('details', '')) or "销售" in (log.get('action_type', '') + log.get('details', ''))):
                finished_steps.add("美工分发")
                break
        # 3. 剪辑分发：剪辑分发运营或分发销售（模糊匹配）
        for log in logs:
            if "剪辑" in log.get('role', '') and "分发" in (log.get('action_type', '') + log.get('details', '')) and ("运营" in (log.get('action_type', '') + log.get('details', '')) or "销售" in (log.get('action_type', '') + log.get('details', ''))):
                finished_steps.add("剪辑分发")
                break
        # 4. 销售领取：销售领取素材（模糊匹配）
        for log in logs:
            if "销售" in log.get('role', '') and "领取" in (log.get('action_type', '') + log.get('details', '')):
                finished_steps.add("销售领取")
                break
        # 5. 运营领取：运营领取素材（模糊匹配）
        for log in logs:
            if "运营" in log.get('role', '') and "领取" in (log.get('action_type', '') + log.get('details', '')):
                finished_steps.add("运营领取")
                break
        # 6. 运营上架：运营添加产品信息
        for log in logs:
            if "运营" in log.get('role', '') and log.get('action_type', '') == "添加产品信息":
                finished_steps.add("运营上架")
                break
        dialog = QDialog(self)
        dialog.setWindowTitle(f"工单详细信息 - {order_data['id']}")
        dialog.resize(800, 700)
        dialog.setMinimumSize(600, 500)
        dialog.setStyleSheet("""
            QDialog { background-color: #23272e; color: #fff; }
            QGroupBox { border: 1.5px solid #3a3f4b; border-radius: 10px; margin-top: 10px; }
            QGroupBox::title { subcontrol-origin: margin; left: 10px; padding: 0 6px; font-size: 17px; font-weight: bold; color: #4fc3f7; }
            QLabel { color: #e0e0e0; font-size: 15px; }
            .field-label { color: #90caf9; font-weight: bold; min-width: 90px; }
            .field-value { color: #fff; font-size: 15px; }
            .step-label { font-size: 15px; font-weight: bold; min-width: 120px; padding: 0 8px; }
            .step-done { color: #4caf50; border-bottom: 3px solid #4caf50; }
            .step-todo { color: #b0bec5; border-bottom: 3px solid #b0bec5; }
            .step-arrow { color: #b0bec5; font-size: 18px; }
            .log-role { color: #4fc3f7; font-weight: bold; }
        ...
        main_layout = QVBoxLayout(dialog)
        # 上方：工单基本信息
        info_group = QGroupBox("工单基本信息")
        info_layout = QFormLayout(info_group)
        info_layout.setLabelAlignment(Qt.AlignRight)
        info_layout.setFormAlignment(Qt.AlignTop)
        def add_info(label, value):
            l = QLabel(label)
            l.setProperty('class', 'field-label')
            v = QLabel(str(value))
            v.setProperty('class', 'field-value')
            info_layout.addRow(l, v)
        add_info("工单ID", order_data['id'])
        add_info("部门", order_data.get('department', ''))
        ...
        add_info("项目类型", order_data.get('project_type', ''))
        add_info("项目内容", order_data.get('project_content', ''))
        ...
        for i, step in enumerate(step_names):
            label = QLabel(step)
            label.setProperty('class', 'step-label')
            if step in finished_steps:
                label.setProperty('class', 'step-label step-done')
                label.setStyleSheet("color: #4caf50; border-bottom: 3px solid #4caf50; font-weight: bold;")
            else:
                label.setProperty('class', 'step-label step-todo')
                label.setStyleSheet("color: #b0bec5; border-bottom: 3px solid #b0bec5; font-weight: bold;")
            step_layout.addWidget(label)
            if i < len(step_names) - 1:
                arrow = QLabel("→")
                arrow.setProperty('class', 'step-arrow')
                arrow.setStyleSheet("color: #b0bec5; font-size: 18px; padding: 0 4px;")
        ...
        logs_widget = QWidget()
        logs_vbox = QVBoxLayout(logs_widget)
        if not logs:
            empty = QLabel("暂无操作日志")
            empty.setStyleSheet("color: #b0bec5; font-size: 14px;")
            logs_vbox.addWidget(empty)
        else:
            for log in logs:
                log_box = QWidget()
                ...
                row1.addWidget(time)
                row1.addStretch()
                log_box_layout.addLayout(row1)
                # 第二行：详情
                details = QLabel(log.get('details', ''))
                details.setProperty('class', 'log-details')
                log_box_layout.addWidget(details)
                logs_vbox.addWidget(log_box)
                line = QFrame()
                line.setFrameShape(QFrame.HLine)
                line.setStyleSheet("color: #374151; background: #374151; height: 1px;")
                logs_vbox.addWidget(line)
        logs_vbox.addStretch()
        logs_widget.setLayout(logs_vbox)
        logs_scroll.setWidget(logs_widget)
        ...
        close_btn.setStyleSheet("background: #1976d2; color: #fff; padding: 8px 32px; border-radius: 6px; font-size: 15px;")
